...
describe("scheduled weather", () => {
  it("keeps observation and forecast times, units, precipitation and attribution", () => {
    const result = prepareWeatherData(observation, forecast, alerts, now);
  ...
  it.each([
    ...
    expect(() => prepareWeatherData(o, f, a, now)).toThrow();
  });
  it("caps freshness by the observation time, not just when the job ran", () => {
    ...
    expect(prepareWeatherData(old, forecast, alerts, now).expiresAt).toBe("2026-09-19T15:30:00.000Z");
  });
});

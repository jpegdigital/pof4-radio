...
export function prepareWeatherData(observation: unknown, forecast: unknown, alerts: unknown, now: number) {
  ...
  if (observed > now + FUTURE_TOLERANCE_MS || updated > now + FUTURE_TOLERANCE_MS)
...
export async function prepareWeather(signal: AbortSignal) {
  const responses = await Promise.all(
    [WEATHER_URLS.observation, WEATHER_URLS.forecast, ALERTS_URL].map(async (url) => {
  ...
  const [observation, forecast, alerts] = responses;
  return {
    ...prepareWeatherData(observation, forecast, alerts, Date.now()),
    evidence: { observation, forecast, alerts },
  };
}

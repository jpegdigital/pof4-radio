...
describe("toElements", () => {
  it("lays out the practice program", () => {
    ...
    expect(els.map((e) => e.kind)).toEqual(["break", "song", "song", "song", "song", "id", "break", "song"]);
    expect(els[0]).toMatchObject({ clip: "break-small", bed: t("bed") });
    ...
    expect(els[5]).toMatchObject({ clip: "legal-id" });
    expect(els[6]).toMatchObject({ clip: "break-big", bed: t("bed") });
    expect(els[7]).toMatchObject({ track: t("5"), talk: { clip: "talkup-5", over: "intro" } });
  });
  it("needs five songs", () => {

 ...
 * and ids only; the tuning stays on the server, where the voice route reads it per segment. The
 * browser remembers which one is picked — a per-browser convenience in localStorage, wrapped in
...
/** An empty roster (no `settings.voices` row yet): the picker shows this and can't open a station. */
